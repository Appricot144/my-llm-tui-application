...
チャット入力欄で `/` から始まるコマンドを実行できます：

- `/init` - プロジェクトの概要（目的・技術スタック・実装対応）を自動生成しキャッシュする
  - ディレクトリ構造とファイル内容を自動分析
  - プロジェクトの目的、使用技術、実装状況をまとめた概要を生成
  - 生成された概要は以降の会話でコンテキストとして活用される

コマンドの実行中はツール使用の確認やファイルアクセスがセキュリティ設定に従って処理されます。

- **差分可視化**: ファイル編集時にコード変更を事前プレビュー表示
- **エージェントによるファイル操作**: 自律的なファイル探索・編集・作成
|---|---|---|
| **Chat** | なし | 純粋な会話 |
...
- **edit_file** — ファイルの指定行範囲編集
- **create_directory** — ディレクトリの作成

**差分プレビュー機能**: ファイル編集ツールの実行前に、変更内容をunified diff形式で表示します。変更が大きい場合や重要なファイルを編集する際に、実際の変更前に内容を確認できるため、安全にファイル編集を行うことができます。
## セキュリティ機能

### 対話型ツール確認

AIがファイルシステムに変更を加える操作を実行する前に、ユーザーに明示的な確認を求めます。
...
ユーザーが `y` を入力すると実行され、それ以外（`N` や Enter）では操作がキャンセルされます。

### ファイルアクセス制限

- **パストラバーサル防止** — プロジェクトルート外へのアクセスを禁止
- **Denylist/Allowlist** — 設定ファイルで特定パスの許可・拒否を制御
## 設定ファイルの使い方
